...
void init(){
	...
	number_of_process = 3;
	push(qarrival, new_process(1,  0, 23, 2), 0); //2 //22
	...
	while(finished_process < number_of_process){
		...
		while(!empty(qdisc_io)){
			...
			if(disc_time + io_time[DISC] <= clock_time){
				...
				front(qdisc_io) -> used_time += io_time[DISC];
		...
		while(!empty(qmagnetic_io)){
			...
			if(magnetic_time + io_time[MAGNETICTAPE] <= clock_time){
				...
				front(qmagnetic_io) -> used_time += io_time[MAGNETICTAPE];
		...
		while(!empty(qprinter_io)){
			...
			if(printer_time + io_time[PRINTER] <= clock_time){
				...
				front(qprinter_io) -> used_time += io_time[PRINTER];
		...
		/*while(!empty(printer) && printer_time + A <= clock_time)
			printer_time = max(printer_time, front())
			front(printer) --> high priority
		...
		*/
		...
		if( !empty(qhigh_priority) ){ //executing process from high priority queue
			...
			pop(qhigh_priority);
			int delta_time = min(quantum, next_process -> total_time - next_process -> used_time);
			if(next_process -> next_io < next_process -> io_qtd // still have ios
				// will do next io after this quantum
				&& delta_time >= next_process -> ios[next_process -> next_io] . time - next_process -> used_time){
				
				delta_time = next_process -> ios[next_process -> next_io] . time - next_process -> used_time;
				
				next_process -> next_io++;
				next_process -> used_time += delta_time;			
				...
				//still needs time to finish		
				clock_time += delta_time;
				if(next_process -> used_time < next_process -> total_time){
			...
			if(next_process -> next_io < next_process -> io_qtd // still have ios
				// will do next io after this quantum
				&& delta_time >= next_process -> ios[next_process -> next_io] . time - next_process -> total_time){
